...
#ifdef CONFIG_F2FS_FIRM_STAT_FS
...
int __init f2fs_init_sysfs(void)
{
	...
	kobject_set_name(&f2fs_kset.kobj, "f2fs-firm");
	...
	if (ret) {
		...
		f2fs_proc_root = proc_mkdir("fs/f2fs-firm", NULL);
	}
	return ret;
}
...
void f2fs_exit_sysfs(void)
{
	kobject_put(&f2fs_feat);
	kset_unregister(&f2fs_kset);
	remove_proc_entry("fs/f2fs-firm", NULL);
	f2fs_proc_root = NULL;
}
