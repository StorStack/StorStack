...
static pthread_mutex_t g_mutex = PTHREAD_MUTEX_INITIALIZER; // XXX: lock initer
static bool g_error;
...
spdk_fio_poll_ctrlrs(void *arg)
{
	...
	while (true) {
		rc = pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &oldstate); // XXX: Set cancel state as DISABLE that threads can run ignoring pthread_cancel signals.
		if (rc != 0) {
		...
		TAILQ_FOREACH(fio_ctrlr, &g_ctrlrs, link) {
			spdk_nvme_ctrlr_process_admin_completions(fio_ctrlr->ctrlr); // XXX: poll
		}

		pthread_mutex_unlock(&g_mutex);

		rc = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &oldstate); // XXX: return to normal state
		if (rc != 0) {
...
get_fio_ctrlr(const struct spdk_nvme_transport_id *trid) // XXX: Select a ctrlr by trid and return
{
	struct spdk_fio_ctrlr	*fio_ctrlr;
...
get_fio_qpair(struct spdk_fio_thread *fio_thread, struct fio_file *f) // XXX: Select a qpair by file and return
{
	struct spdk_fio_qpair	*fio_qpair;
...
_nvme_get_host_buffer_sector_size(struct spdk_nvme_ns *ns, uint32_t io_flags) // XXX: get buffer size
{
	  ...
	  struct spdk_nvme_ctrlr *ctrlr, const struct spdk_nvme_ctrlr_opts *opts)
{
	...
	p = strstr(f->file_name, "ns="); // XXX: find sub string
	if (p != NULL) {
		tmp = spdk_strtol(p + 3, 10); // XXX: string -> long int, to get namespace id
		if (tmp <= 0) {
	...
	if (!fio_ctrlr) {
		...
		fio_ctrlr = calloc(1, sizeof(*fio_ctrlr)); // XXX: with memset all zero
		if (!fio_ctrlr) {
...
spdk_fio_setup(struct thread_data *td)
{
	...
	pthread_mutex_lock(&g_mutex); // XXX: lock by cred 
	...
	if (!g_spdk_env_initialized) { // XXX: if not inited
		spdk_env_opts_init(&opts);
		opts.name = "fio"; // name
		opts.mem_size = fio_options->mem_size;
		opts.shm_id = fio_options->shm_id; // XXX: share id
		...
		if (spdk_env_init(&opts) < 0) { // XXX: init spdk env
		...
		g_spdk_env_initialized = true; // XXX: init spdk flag
		spdk_unaffinitize_thread(); // XXX: CPU affinity from others
	...
	for_each_file(td, f, i) {
		...
		pthread_mutex_lock(&g_mutex);
		fio_ctrlr = get_fio_ctrlr(&trid); // XXX: controller setup
		pthread_mutex_unlock(&g_mutex);
		if (fio_ctrlr) {
...
{
	td->orig_buffer = spdk_dma_zmalloc(total_mem, NVME_IO_ALIGN, NULL); // XXX: Align is 4096 to tie to NVMe
...
{
	struct spdk_fio_request *fio_req = io_u->engine_data; // XXX: ptr for struct

	if (fio_req) {
		assert(fio_req->io == io_u);
		spdk_dma_free(fio_req->md_buf); // XXX: spdk_dma_malloc()
		free(fio_req); // struct free (normal) - compared with above
		io_u->engine_data = NULL;
	}
}
