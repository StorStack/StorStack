...
struct spdk_nvme_cmd
{
	...
	/* dword 2-3: StorStack specific */
	union
	{
		union
		{
			uint16_t pid;	/* ss partition id */
			uint16_t fspid; /* ss file system partition id */
			uint32_t uid;	/* ss klib user id for auth */
		};
		union
		{
			uint32_t rsvd2;
	...
	/**
	 * StorStack custom command structure
	 */
	union
	{
		union
		{
			uint32_t cdw10_ss_path_len;		   // pathname.length
			uint32_t cdw10_ss_file_fd;		   // file description
			uint32_t cdw10_ss_oldfilename_len; // with "\0"
			uint32_t cdw10;					   // raw data
		};
	...
	union
	{
		union
		{
			uint32_t cdw11_ss_open_flags;	   // open file flags "O_XXXXX"
			uint32_t cdw11_ss_buffer_len;	   // byte length
			uint32_t cdw11_ss_newfilename_len; // with "\0"
			uint32_t cdw11;
		};
	...
	union
	{
		uint32_t cdw12_ss_file_mode; // open file mode
		uint32_t cdw12_ss_offset;	 // loff_t long int 32bits
		uint32_t cdw12_ss_admin_core_id;	// admin cmd core id
		uint32_t cdw12;
	};
	...
	union
	{
		uint32_t cdw13_ss_whence;		 // lseek whence
		uint32_t cdw13_ss_allocate_mode; // allocate mode
		uint32_t cdw13_ss_file_len;		 // ftruncate file length
		uint32_t cdw13;
	};
	...
	union
	{
		uint32_t cdw14_ss_space_len; // allocate space length
		uint32_t cdw14;
	};

	union
	{
		uint32_t cdw15_token; /* StorStack klib operation token */
		uint32_t cdw15_core_id; /* StorStack queue core binding */
		uint32_t cdw15;
	};
};
...
struct spdk_nvme_cpl {
	/* dword 0 */
	uint32_t		cdw0;	/* StorStack return value */

	/* dword 1 */
	uint32_t		cdw1;	/* StorStack permission check fallback */
...
enum spdk_nvme_command_specific_status_code {
	...
	SPDK_NVME_SC_CMD_SIZE_LIMIT_SIZE_EXCEEDED	= 0x83,
	    SPDK_NVME_SC_SS_PERM_DENY           = 0x90,
...
enum spdk_nvme_admin_opcode {
	...
	/**
	 * StorStack custom admin operations
	*/
	SPDK_NVME_OPC_SS_PART_INIT			= 0xC0, // 1 10000 00
	SPDK_NVME_OPC_SS_SKEY			= 0xC1, // 1 10000 01
...
enum spdk_nvme_nvm_opcode {
	...
	/**
	 * StorStack custom io operations
	*/
	SPDK_NVME_OPC_SS_OPEN			= 0xC1, // 1 10000 01 host -> dev
	SPDK_NVME_OPC_SS_CLOSE			= 0xC0, // 1 10000 00 ---
	SPDK_NVME_OPC_SS_WRITE			= 0xC5, // 1 10001 01 host -> dev
	SPDK_NVME_OPC_SS_READ			= 0xC6, // 1 10001 10 dev -> host
	SPDK_NVME_OPC_SS_PWRITE			= 0xC9, // 1 10010 01 host -> dev
	SPDK_NVME_OPC_SS_PREAD			= 0xCA, // 1 10010 10 dev -> host
	SPDK_NVME_OPC_SS_LSEEK			= 0xCC, // 1 10011 00 ---
	SPDK_NVME_OPC_SS_UNLINK			= 0xD1, // 1 10100 01 host -> dev
	SPDK_NVME_OPC_SS_FSYNC			= 0xD4, // 1 10101 00 --- 
	SPDK_NVME_OPC_SS_FALLOCATE		= 0xD8, // 1 10110 00 --- 
	SPDK_NVME_OPC_SS_FTRUNCATE		= 0xDC, // 1 10111 00 ---
	SPDK_NVME_OPC_SS_RENAME			= 0xE1, // 1 11000 01 host -> dev
	SPDK_NVME_OPC_SS_MKDIR          = 0xE5, // 1 11001 01 host -> dev
    SPDK_NVME_OPC_SS_STAT       	= 0xE7, // 1 11001 11 host <> dev
	SPDK_NVME_OPC_SS_LAT_WITH_RET   = 0xE0, // 1 11000 00 ---
	SPDK_NVME_OPC_SS_LAT_WITH_RET_R = 0xEA, // 1 11010 10 dev -> host
	SPDK_NVME_OPC_SS_LAT_WITH_RET_W = 0xE9, // 1 11010 01 host -> dev
	SPDK_NVME_OPC_SS_LAT_NO_RET     = 0xE4, // 1 11001 00 ---
	SPDK_NVME_OPC_SS_RMDIR          = 0xED, // 1 11011 01 host -> dev
};
