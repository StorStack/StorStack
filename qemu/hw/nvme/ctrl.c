...
// extern 
uint32_t latency_sim_process_sq_ns = 0;
...
static const uint32_t nvme_cse_acs[256] = {
    ...
    // StorStack admin commands
    [NVME_ADM_CMD_SS_PART_INIT]     = NVME_CMD_EFF_CSUPP,
    [NVME_ADM_CMD_SS_SKEY]          = NVME_CMD_EFF_CSUPP,
};
...
static const uint32_t nvme_cse_iocs_nvm[256] = {
    ...
    // StorStack io commands
    // because we do not apply real op to the nvme device, so use no LBCC
    [NVME_CMD_SS_OPEN]              = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_CLOSE]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_WRITE]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_READ]              = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_PWRITE]            = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_PREAD]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_LSEEK]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_UNLINK]            = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_FSYNC]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_FALLOCATE]         = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_FTRUNCATE]         = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_RENAME]            = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_MKDIR]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_RMDIR]             = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_STAT]              = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_LAT_WITH_RET]      = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_LAT_WITH_RET_R]      = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_LAT_WITH_RET_W]      = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_LAT_NO_RET]        = NVME_CMD_EFF_CSUPP,

    // TODO: the following two cmd are for test, remove them
    [NVME_CMD_SS_HELLOW]            = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_SS_HELLOR]            = NVME_CMD_EFF_CSUPP,
};
...
static const uint32_t nvme_cse_iocs_zoned[256] = {
    ...
    // StorStack io commands
    // NOTE: Do not use ZNS feature
};

// static void __nvme_check_and_update(void* opaque); // XXX: callback for timer of sq
                       ...
                       NvmeCmd *cmd)
{
    ...
    case NVME_PSDT_PRP:
        // printf("[QEMU_nvme_map_dptr] Select PRP method.\n"); // StorStack Debug
    ...
    case NVME_PSDT_SGL_MPTR_SGL:
        // printf("[QEMU_nvme_map_dptr] Select SGL method: %d\n", NVME_CMD_FLAGS_PSDT(cmd->flags)); // StorStack Debug
...
// Transfer from DEVICE(QEMU) to HOST(VM) 
uint16_t nvme_c2h(NvmeCtrl *n, void *ptr, uint32_t len,
                                NvmeRequest *req)
{
    uint16_t status;
...
// Transfer from HOST(VM) to DEVICE(QEMU) 
uint16_t nvme_h2c(NvmeCtrl *n, void *ptr, uint32_t len,
                                NvmeRequest *req)
{
    uint16_t status;
    status = nvme_map_dptr(n, &req->sg, len, &req->cmd);
...
static void nvme_enqueue_req_completion(NvmeCQueue *cq, NvmeRequest *req)
{
    ...
    // https://lore.kernel.org/all/YsRwyMONg0+mHVsL@apples/
    if (req->sq->ioeventfd_enabled) {
        /* Post CQE directly since we are in main loop thread */
        nvme_post_cqes(cq);
    } else if (cq->ctrl->core_enabled) {
        // multi core support
        nvme_post_cqes(cq);
    } else {
...
static uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    // StorStack
    case NVME_CMD_SS_OPEN:
        return nvme_ss_open(n, req);	 
    case NVME_CMD_SS_CLOSE:
        return nvme_ss_close(n, req);	 
    case NVME_CMD_SS_WRITE:
        return nvme_ss_write(n, req);	 
    case NVME_CMD_SS_READ:
        return nvme_ss_read(n, req);	 
    case NVME_CMD_SS_PWRITE:
        return nvme_ss_pwrite(n, req);   
    case NVME_CMD_SS_PREAD:
        return nvme_ss_pread(n, req);	 
    case NVME_CMD_SS_LSEEK:
        return nvme_ss_lseek(n, req);	 
    case NVME_CMD_SS_UNLINK:
        return nvme_ss_unlink(n, req);	 
    case NVME_CMD_SS_FSYNC:
        return nvme_ss_fsync(n, req);	 
    case NVME_CMD_SS_FALLOCATE:
        return nvme_ss_fallocate(n, req);
    case NVME_CMD_SS_FTRUNCATE:
        return nvme_ss_ftruncate(n, req);
    case NVME_CMD_SS_RENAME:
        return nvme_ss_rename(n, req);
    case NVME_CMD_SS_MKDIR:
        return nvme_ss_mkdir(n, req);
    case NVME_CMD_SS_RMDIR:
        return nvme_ss_rmdir(n, req);
    case NVME_CMD_SS_STAT:
        return nvme_ss_stat(n, req);
    case NVME_CMD_SS_LAT_WITH_RET:
        return nvme_ss_lat_with_ret(n, req);
    case NVME_CMD_SS_LAT_WITH_RET_R:
        return nvme_ss_lat_with_ret_r(n, req);
    case NVME_CMD_SS_LAT_WITH_RET_W:
        return nvme_ss_lat_with_ret_w(n, req);
    case NVME_CMD_SS_LAT_NO_RET:
        return nvme_ss_lat_no_ret(n, req);
    // TODO: the following two cmd are for test, remove them
    case NVME_CMD_SS_HELLOW:
        return nvme_ss_hellow(n, req);
    case NVME_CMD_SS_HELLOR:
        return nvme_ss_hellor(n, req);
    default:
        assert(false);
    }
...
static void nvme_free_sq(NvmeSQueue *sq, NvmeCtrl *n)
{
    ...
    if (n->core_enabled)
    {
        sq->core->sq[sq->sqid] = NULL;
        sq->core->sqnum--;
        qemu_bh_delete(sq->core_bh);
    }
    timer_free(sq->timer); // XXX: REF timer free
...
static uint16_t nvme_del_sq(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    if (!nvme_check_cqid(n, sq->cqid)) {
        ...
        QTAILQ_FOREACH_SAFE(r, &cq->req_list, entry, next) {
            if (r->sq == sq) {
                // timer_free(r->lat_timer); // XXX: timer free
...
static void nvme_init_sq(NvmeSQueue *sq, NvmeCtrl *n, uint64_t dma_addr,
                         uint16_t sqid, uint16_t cqid, uint16_t size, uint32_t coreid)
{
    int i;
    NvmeCQueue *cq;
    ...
    for (i = 0; i < sq->size; i++) {
        sq->io_req[i].sq = sq;
        // AioContext *aio = iothread_get_aio_context(sq->core->thread);
        // aio_timer_init(aio, sq->io_req[i].lat_timer, QEMU_CLOCK_VIRTUAL, SCALE_NS, __nvme_check_and_update, sq); // XXX: timer init
        // TODO: ref 4495, init timer
    ...
    if (sq->ctrl->core_enabled)
    {
        sq->core = n->cores[coreid];
        sq->core->sq[sqid] = sq;
        sq->core->sqnum += 1;
        AioContext *aio = iothread_get_aio_context(sq->core->thread);
        // printf("init sq %d at coreid %d threadid %d\n", sqid, coreid, sq->core->thread->thread_id);
        sq->core_bh = aio_bh_new(aio, nvme_process_sq, sq);
    }
}
...
static uint16_t nvme_create_sq(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    uint32_t coreid = le32_to_cpu(c->rsvd12[3]); // FIXME: core-id by StorStack
    ...
    if (n->core_enabled){
        // TODO: remove this
        // coreid = 0;
        // printf("prepare to init ss sq %d at core %d\n", sqid, coreid);
        nvme_init_sq(sq, n, prp1, sqid, cqid, qsize + 1, coreid);
    } else {
        nvme_init_sq(sq, n, prp1, sqid, cqid, qsize + 1, 0);
    }
    return NVME_SUCCESS;
}
...
static void nvme_free_cq(NvmeCQueue *cq, NvmeCtrl *n)
{
    ...
    if (n->core_enabled)
    {
        cq->core->cq[cq->cqid] = NULL;
        cq->core->cqnum--;
        qemu_bh_delete(cq->core_bh);
    }
    timer_free(cq->timer);
...
static void nvme_init_cq(NvmeCQueue *cq, NvmeCtrl *n, uint64_t dma_addr,
                         ...
                         uint16_t irq_enabled, uint32_t coreid)
{
    int ret;
    ...
    if (cq->ctrl->core_enabled)
    {
        cq->core = n->cores[coreid];
        cq->core->cq[cqid] = cq;
        cq->core->cqnum += 1;
        AioContext *aio = iothread_get_aio_context(cq->core->thread);
        // printf("init cq %d at coreid %d threadid %d\n", cqid, coreid, cq->core->thread->thread_id);
        cq->core_bh = aio_bh_new(aio, nvme_post_cqes, cq);
    }
}
...
static uint16_t nvme_create_cq(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    if (n->core_enabled){
        // TODO: remove this
        // coreid = 0;
        // printf("prepare to init ss cq %d at core %d\n", cqid, coreid);
        nvme_init_cq(cq, n, prp1, cqid, vector, qsize + 1,
                 NVME_CQ_FLAGS_IEN(qflags), coreid);
    } else {
        nvme_init_cq(cq, n, prp1, cqid, vector, qsize + 1,
                 NVME_CQ_FLAGS_IEN(qflags), 0);
    }
...
static uint16_t nvme_admin_cmd(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_ADM_CMD_SS_PART_INIT:
        return nvme_ss_adm_part_init(n, req);
    case NVME_ADM_CMD_SS_SKEY:
        return nvme_ss_adm_skey(n, req);
    default:
        assert(false);
    }
...
static void nvme_process_sq(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list))) {
        ...
        simulate_latency(latency_sim_process_sq_ns); // XXX: Latency for sq requests
...
//             }
//             nvme_inc_sq_head(sq);
//             // TODO: move below to a func, check core_enabled, add timer_mod here.
//             // XXX: coding
//             req = QTAILQ_FIRST(&sq->req_list);
//             QTAILQ_REMOVE(&sq->req_list, req, entry);
//             QTAILQ_INSERT_TAIL(&sq->out_req_list, req, entry);
...
//     }
//     else {
//         timer_mod(QTAILQ_FIRST(&sq->req_list)->lat_timer, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + 500); // latency
//     }
// }

// // XXX: coding callback func
// static void __nvme_check_and_update(void* opaque)
// {
//     NvmeSQueue *sq = opaque;
//     NvmeCtrl *n = sq->ctrl;
//     NvmeCQueue *cq = n->cq[sq->cqid];
...
static void nvme_free_ctrl_core(NvmeCtrlCore *c, NvmeCtrl *n)
{
    g_free(c->sq);
    g_free(c->cq);
    iothread_destroy(c->thread);
    g_free(c);
}
...
static int nvme_start_ctrl(NvmeCtrl *n)
{
    ...
    nvme_init_cq(&n->admin_cq, n, acq, 0, 0, NVME_AQA_ACQS(aqa) + 1, 1, 0);
    nvme_init_sq(&n->admin_sq, n, asq, 0, 0, NVME_AQA_ASQS(aqa) + 1, 0);
    ...
    if (((addr - 0x1000) >> 2) & 1) {
        ...
        if (start_sqs) {
            ...
            QTAILQ_FOREACH(sq, &cq->sq_list, entry) {
                if (n->core_enabled) {
                    // printf("sq %d schedule core bh\n", sq->sqid);
                    qemu_bh_schedule(sq->core_bh);
                } else {
                    timer_mod(sq->timer, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + 500);
                }
            }
            if (n->core_enabled) {
                // printf("cq %d schedule core bh\n", cq->cqid);
                qemu_bh_schedule(cq->core_bh);
            } else {
                timer_mod(cq->timer, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + 500);
            }
        }
...
static void nvme_check_constraints(NvmeCtrl *n, Error **errp)
{
    ...
    if (n->params.max_cores > 8)
    {
        error_setg(errp, "max_cores more than 8 is not supported");
        return;
    }
    if (n->params.ctrl_cores > 0 && n->params.ioeventfd)
    {
        error_setg(errp, "ctrl_cores are not supported to work with ioeventfd");
        return;
    }
    
    if (n->params.ctrl_cores != n->params.len_cthreads || 
        n->params.ctrl_cores > n->params.max_cores) 
    {
        error_setg(errp, "ctrl_cores should be equal to the length of core_threads and no more than max_cores");
        return;
    }
    if (n->params.len_bind_cpus > 0 && 
        n->params.len_bind_cpus != n->params.ctrl_cores)
    {
        error_setg(errp, "len-bind_cpus should be equal to ctrl_cores");
        return;
    }
...
static void nvme_init_state(NvmeCtrl *n)
{
    ...
    n->cores = g_new0(NvmeCtrlCore *, n->params.max_cores + 1);
...
static void nvme_init_ctrl_core(NvmeCtrl *n, NvmeCtrlCore *c, char *id, int cpu) {
    // core init
    c->ctrl = n;
    c->sqnum = 0;
    c->cqnum = 0;
    c->sq   = g_new0(NvmeSQueue *, n->params.max_ioqpairs + 1);
    c->cq   = g_new0(NvmeCQueue *, n->params.max_ioqpairs + 1);
    c->thread = iothread_by_id(id);
    // bind ctrl_core to host cpu
    if (cpu >= 0)
    {
        printf("binding thread %d to cpu %d\n", c->thread->thread_id, cpu);
        int num_cores = sysconf(_SC_NPROCESSORS_ONLN);
        if (cpu >= num_cores) goto err;
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(cpu, &cpuset);
        if (sched_setaffinity(c->thread->thread_id, sizeof(cpuset), &cpuset))
            goto err;
    }
    return;

err:
    printf("failed binding thread %s to cpu %d\n", id, cpu);
    return;
}
...
static void nvme_init_ctrl(NvmeCtrl *n, PCIDevice *pci_dev)
{
    ...
    id->sgls = cpu_to_le32(NVME_CTRL_SGLS_NOT_SUPPORT); // StorStack: no SGL support
    ...
    nvme_ss_init();
    // StorStack multicore support
    int cnum = n->params.ctrl_cores;
    n->core_enabled = (cnum > 0);
    int cpu_idx = n->params.bind_cpu_from;
    ...
    for (int i = 0; i < cnum; i++)
    {
        ...
        if (n->params.bind_cpu_from >= 0)
        {
            cpu = cpu_idx++;
        } 
        else if (n->params.len_bind_cpus > 0)
        {
            cpu = n->params.bind_cpus[i];
        }
        nvme_init_ctrl_core(n, c, n->params.cthreads[i], cpu);
        n->cores[i] = c;
    }
}
...
static void nvme_exit(PCIDevice *pci_dev)
{
    ...
    // qcrypto_hmac_free(nvme_ss_hmac);
    ...
    int cnum = n->params.ctrl_cores ? n->params.ctrl_cores : 1;
    for (i = 0; i < cnum; i++)
    {
        if (n->cores[i] != NULL) {
            nvme_free_ctrl_core(n->cores[i], n);
        }
    }
    g_free(n->cores);

    for (int i = 0; i < n->params.len_cthreads; i++)
    {
        g_free(n->params.cthreads[i]);
    }
...
static void get_iothread(Object *obj, Visitor *v, const char *name,
                       void *opaque, Error **errp)
{
    Property *prop = opaque;
    char **tid = object_field_prop_ptr(obj, prop);

    visit_type_str(v, name, tid, errp);
}

static void set_iothread(Object *obj, Visitor *v, const char *name,
                       void *opaque, Error **errp)
{
    Property *prop = opaque;
    char **tid = object_field_prop_ptr(obj, prop);
...
const PropertyInfo prop_iothread = {
    .name  = "iothread",
    .get   = get_iothread,
    .set   = set_iothread,
};
...
static Property nvme_props[] = {
    ...
    // DEFINE_PROP_STRING("ss_bdev_path", NvmeCtrl, params.ss_bdev_path),
    ...
    DEFINE_PROP_UINT8("ctrl_cores", NvmeCtrl,
                      params.ctrl_cores, 0),
    DEFINE_PROP_UINT8("max_cores", NvmeCtrl, params.max_cores, 8),
    DEFINE_PROP_INT32("bind_cpu_from", NvmeCtrl, params.bind_cpu_from, -1),
    // DEFINE_PROP_LINK("core-thread0", NvmeCtrl, params.thread, TYPE_IOTHREAD,
    //                  IOThread *),
    DEFINE_PROP_ARRAY("core_threads", NvmeCtrl, params.len_cthreads, 
                      params.cthreads, prop_iothread, char*),
    DEFINE_PROP_ARRAY("bind_cpus", NvmeCtrl, params.len_bind_cpus, 
                      params.bind_cpus, qdev_prop_uint32, uint32_t),
