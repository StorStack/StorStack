...
enum NvmeAdminCommands {
    ...
    // Initialize partition: load FS and mount the partition. 
    NVME_ADM_CMD_SS_PART_INIT   = 0xC0, // 1 10000 00
    // Update secret key for crediential generation.
    NVME_ADM_CMD_SS_SKEY        = 0xC1, // 1 10000 01
};
...
enum NvmeIoCommands {
    ...
    // StorStack file operations.
	NVME_CMD_SS_OPEN	    	= 0xC1, // 1 10000 01 host -> dev
	NVME_CMD_SS_CLOSE	    	= 0xC0, // 1 10000 00 ---
	NVME_CMD_SS_WRITE	    	= 0xC5, // 1 10001 01 host -> dev
	NVME_CMD_SS_READ	    	= 0xC6, // 1 10001 10 dev -> host
	NVME_CMD_SS_PWRITE      	= 0xC9, // 1 10010 01 host -> dev
	NVME_CMD_SS_PREAD	    	= 0xCA, // 1 10010 10 dev -> host
	NVME_CMD_SS_LSEEK	    	= 0xCC, // 1 10011 00 ---
	NVME_CMD_SS_UNLINK	        = 0xD1, // 1 10100 01 host -> dev
	NVME_CMD_SS_FSYNC	    	= 0xD4, // 1 10101 00 --- 
	NVME_CMD_SS_FALLOCATE   	= 0xD8, // 1 10110 00 --- 
	NVME_CMD_SS_FTRUNCATE   	= 0xDC, // 1 10111 00 ---
	NVME_CMD_SS_RENAME      	= 0xE1, // 1 11000 01 host -> dev
    NVME_CMD_SS_MKDIR           = 0xE5, // 1 11001 01 host -> dev
    NVME_CMD_SS_STAT            = 0xE7, // 1 11001 11 host <> dev
    NVME_CMD_SS_LAT_WITH_RET    = 0xE0, // 1 11000 00 ---
    NVME_CMD_SS_LAT_WITH_RET_R  = 0xEA, // 1 11010 10 dev -> host
	NVME_CMD_SS_LAT_WITH_RET_W  = 0xE9, // 1 11010 01 host -> dev
    NVME_CMD_SS_LAT_NO_RET      = 0xE4, // 1 11001 00 ---
    NVME_CMD_SS_RMDIR           = 0xED, // 1 11011 01 host -> dev

    // TODO: the following two cmd are for test, remove them
    NVME_CMD_SS_HELLOW          = 0xFD, // 1 11111 01 host -> dev
    NVME_CMD_SS_HELLOR          = 0xFE, // 1 11111 01 host -> dev
};
...
typedef struct QEMU_PACKED NvmeCreateCq {
    ...
    uint16_t    cq_flags;
    uint16_t    irq_vector;
    uint32_t    rsvd12[4]; // 32*4, cdw12 - cdw15
} NvmeCreateCq;
...
typedef struct QEMU_PACKED NvmeSsCmd {
    uint8_t     opcode;
    uint8_t     flags;  // fuse*2 + rsvd*4 + psdt*2
    uint16_t    cid;
    uint32_t    nsid;
    ...
    uint64_t    mptr;
    NvmeCmdDptr dptr;
    uint32_t    cdw10;  // ss file operation arg0
    uint32_t    cdw11;  // ss file operation arg1
    uint32_t    cdw12;  // ss file operation arg2
    uint32_t    cdw13;  // ss file operation arg3
    uint32_t    cdw14;  // ss file operation arg4
    uint32_t    cdw15;  // ss file operation arg5, token
} NvmeSsCmd;
...
typedef struct QEMU_PACKED NvmeCqe {
    uint32_t    result;
    uint32_t    dw1;        // ss open return inode
    uint16_t    sq_head;
    uint16_t    sq_id;
    uint16_t    cid;
    uint16_t    status;
...
enum NvmeStatusCodes {
    ...
    // 0x84 to 0xb7 is reserved in the spec
    // SS Error Codes, we are using 0x90 to 0xa0
    NVME_SS_PERM_DENY           = 0x90,
};
...
#define NVME_CTRL_SGLS_NOT_SUPPORT         (0x0 <<  0) // No Support for SGL
