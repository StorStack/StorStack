...
flowop_start(threadflow_t *threadflow)
{
	...
	ss_init_qpair(); // FIXME: TEST: qpair fix
	// printf("[flowop_start] ss_init_spdk ok!\n"); // StorStack Debug
	...
	if (threadflow->tf_attrs & THREADFLOW_USEISM) {
		...
		filebench_log(LOG_INFO, "[threadflow] using ipc_ismalloc\n"); // TODO: REMOVE THIS, for DEBUG
	} else {
		threadflow->tf_mem = ss_buf_init(memsize); // XXX: DMA buffer init
		filebench_log(LOG_INFO, "[threadflow] using ss_buf_init\n"); // TODO: REMOVE THIS, for DEBUG
	}
...
{
	//printf("[flowop_init] start!\n"); // StorStack Debug

	ss_init_spdk();
	ss_init_qpair();
	printf("[flowop_init] ss_init_spdk ok!\n"); // StorStack Debug
...
flowop_delete(flowop_t **flowoplist, flowop_t *flowop)
{
	...
	ss_fini_qpair();
	// ss_fini_spdk(); // FIXME: SPDK finish and destroy
