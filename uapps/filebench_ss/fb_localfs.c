 ...
 * StorStack ULIBSS
 * 
 * All functions below are replaced with StorStack functions.
 */
#include "../../ulibss/klibss.h"
#include "../../ulibss/ulibss.h"
...
{
	return (ss_pread(fd->fd_num, iobuf, iosize, fileoffset)); // XXX: ss
}
...
{
	return (ss_read(fd->fd_num, iobuf, iosize)); // XXX: ss
}
...
{
	printf("init pid: %d tid: %d\n", getpid(), syscall(SYS_gettid));
	if ((fd->fd_num = ss_open(path, flags, perms)) < 0) // XXX: ss
		return (FILEBENCH_ERROR);
	else
		return (FILEBENCH_OK);
}
...
{
	return (readlink(path, buf, buf_size)); // TODO: ss
}
...
{
	return (ss_fsync(fd->fd_num)); // XXX: ss
}
...
{
	return (ss_lseek64(fd->fd_num, offset, whence)); // XXX: ss
}
...
{
	return (ss_close(fd->fd_num)); // XXX: ss
}
...
{
	return (ss_mkdir(path, perm)); // XXX: ss
}
...
{
	return (fstat64(fd->fd_num, statbufp)); // TODO: ss
}
...
{
	return (ss_stat(path, statbufp)); // XXX: ss
}
...
{
	return (ss_pwrite(fd->fd_num, iobuf, iosize, offset)); // XXX: ss
}
...
{
	return (ss_write(fd->fd_num, iobuf, iosize)); // XXX: ss
}
...
{
	return (link(existing, new)); // TODO: ss
}
...
{
	return (symlink(existing, new)); // TODO: ss
}
...
{
	return (access(path, amode)); // TODO: ss
}
